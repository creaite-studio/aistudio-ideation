...
| **Gumroad** | 大型バンドル販売・アフィリ展開 | **$0**（手数料~13%/件） |
| **合計** | | **~$19/月** |
...
| SEOブログ | Ghost Pro | $25 | ★★★★☆ |
| SNSスケジュール | Buffer | $18 | ★★★★☆ |
| デジタル販売（小額） | Ko-fi Gold | $6 | ★★★☆☆ |
| デジタル販売（バンドル）| Gumroad | $0（手数料~13%） | ★★★★☆ |
...
月次で自動実行:
  ...
  □ Gumroadのアフィリ成約率確認 → コミッション率調整（20〜30%）
  □ KaryaKarsaの無料/有料エピソード比率の最適化
  □ 季節商品の告知タイミング最適化
```

#### Ko-fi vs Gumroad 役割分担

| | Ko-fi | Gumroad |
|--|-------|---------|
| **対象商品** | PDFガイド・チップ・応援 | 大型バンドル・年間ガイド |
| **価格帯** | $5〜8（衝動買いライン） | $15〜25（熱狂ファン向け） |
| **手数料** | 0%（Gold $6/月） | ~13%（月額なし） |
| **強み** | シンプル・コミュニティ感 | アフィリプログラム・税務自動対応（VAT/GST） |
| **弱み** | 発見性ゼロ | 手数料がやや高め |
| **自動化** | 手動（月数回） | 半自動（アフィリ経由で受動的に売れる） |

**Gumroad商品ラインナップ（候補）**
```
① 72 Seasons Complete Annual Guide  $20
   → 全72候を1冊にまとめたデジタルブック
   → Ko-fi季節PDF4冊（$24相当）より安い設定で価値訴求

② Hobonichi Complete Setup Guide    $15
   → 初心者向け完全ガイド（選び方・カバー・デコ方法）
   → BunguブログのSEO記事から自然に誘導

アフィリエイト設定:
  slow living / Japanese culture 系英語クリエイターに登録依頼
  コミッション: 20〜30%（業界標準）
  Gumroadのディスカバリー（marketplace）にも自動掲載
  → 完全受動的な販売チャネル追加
```
...
Phase 1（3ヶ月後）:
  ...
  Ko-fi季節PDF × 15本 × $5           = $75
  Gumroad Annual Guide × 5本 × $20   = $100（アフィリ手数料除く ~$87）
  小計:                               $612

Phase 2（6ヶ月後）:
  有料購読 × 200人 × $9              = $1,800
  Ko-fi デジタル商品                 = $200
  Gumroad Annual Guide × 20本 × $20  = $400（アフィリ手数料除く ~$348）
  小計:                               $2,348
```
...
Phase 1（記事10本）:
  ...
  Gumroad Hobonichi Guide × 3本 × $15 = $45（アフィリ手数料除く ~$39）
  小計:                                $139

Phase 2（記事30本）:
  Amazonアフィリ 月300転換 × 平均$2   = $600
  Ko-fi セットアップPDF × 20本        = $160
  Gumroad バンドル × 15本 × $18       = $270（アフィリ手数料除く ~$235）
  ＋ Gumreadアフィリ経由（受動的）    = $50
  小計:                                $1,045
```
...
```
Phase 1（3ヶ月後）:
  $612 + $139 + $150 - $19 = 月+$882

Phase 2（6ヶ月後）:
  $2,348 + $1,045 + $850 - $157 = 月+$4,086

Phase 3（1年後）:
  Gumroadアフィリ受動収益 + スポンサー・コラボ追加で月$6,000+
```

> **Gumroadの手数料について**: 売上の約13%が手数料。ただしVAT/GST（欧州・インド等）を自動処理するため日本からの全世界販売が安心して行える。月額固定費なし。
